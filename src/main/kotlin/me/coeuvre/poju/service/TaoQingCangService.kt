...
class TaoQingCangService(@Autowired val juLikeFlowManager: JuLikeFlowManager, @Autowired val taoQingCangClient: TaoQingCangClient) {
    ...
    fun publishItems(request: PublishItemsRequest): Mono<XSSFWorkbook?> {
        ...
        return taoQingCangClient.queryItems(QueryItemsRequest(request.tbToken, request.cookie2, request.sg, request.activityEnterId, request.itemStatusCode, request.actionStatus, 1, 0))
            .map { it.totalItem }
            .flatMapMany { totalCount ->
                ...
                (1..pageCount).map { currentPage ->
                    ...
                    taoQingCangClient.queryItems(queryItemsRequest).map { Triple(totalCount, queryItemsRequest, it) }
                }
            }
            ...
            .flatMapSequential({ (index, count, item) ->
                ...
                taoQingCangClient.publishItem(PublishItemRequest(request.tbToken, request.cookie2, request.sg, item.juId))
